...
void main() {
  ...
  float hebra = smoothstep(0.26 + blando * 2.0, 0.0, abs(fract(a * 7.0 + 0.5) - 0.5))
              * abanico * smoothstep(0.85 + blando, 0.2, r) * smoothstep(0.0, 0.08, r);
  float puntas = smoothstep(0.12 + blando, 0.0, abs(r - 0.78)) * abanico * 0.8;
  float pico = smoothstep(0.022 + blando, 0.0, abs(q.x)) * step(-0.75, q.y) * step(q.y, 0.15);
  float aquenio = smoothstep(0.1 + blando, 0.0, length((q - vec2(0.0, -0.82)) * vec2(3.0, 1.2)));
  // el vilano es lo que se tiene que leer; el pico queda sutil para no parecer una rayita
  float blanco = max(hebra * 0.9, puntas);
  float pardo = max(pico * 0.35, aquenio);
  // gris cálido y no blanco: sobre el crema de la página un vilano blanco desaparece
  vec3 vilano = mix(vec3(0.58, 0.56, 0.51), vec3(0.74, 0.72, 0.67), puntas);
